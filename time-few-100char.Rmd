...
author: "Jobson Lucas Dias, José Benardi Nunes, Rafael Albuquerque"
output: html_document
---

Neste arquivo, iremos comparar os dados das medições Log1TIME e Log2TIME.
...
Como a api utilizada usa como base para o cálculo dos milissegundos a meia noite de janeiro de 1970 UTC, e portanto, o tempo em milissegundos dará um valor muito grande, não é tão útil colocarmos no gráfico os valores dos milissegundos em si, pois não teriam tanta relevância. É mais adequado, no entanto, colocarmos o valor da diferença, que está medida em milissegundos.
...
names(time_few_2lvls_dif) = c('dif_tempo_add', 'dif_tempo_del')
...
```{r, echo=FALSE}
leitura = 1:60
diferenca_em_ms_add = time_few_2lvls_dif$dif_tempo_add
g=ggplot(time_few_2lvls_dif, 
       aes(y = diferenca_em_ms_add, x = leitura)) +
  geom_point() + 
  labs(title = "Gráfico para operação de add com 100 caracteres e poucos users",  y = "Intervalo de tempo - add(ms)", x = "Nº da leitura")
ggplotly(g)
...
```{r, echo=FALSE}
diferenca_em_ms_del = time_few_2lvls_dif$dif_tempo_del
g=ggplot(time_few_2lvls_dif, 
       aes(y = diferenca_em_ms_del, x = leitura)) +
  geom_point() + 
  labs(title = "Gráfico para operação de deletar com 100 caracteres e poucos users",  y = "Intervalo de tempo - deletar(ms)", x = "Nº da leitura")
ggplotly(g)
...
Abaixo veremos a média, mediana, e desvio padrão, além do intervalo de confiança(i.c.) com 95% de confiança para os dados dessa operação. Também vamos avaliar o grau de representatividade da média através do coeficiente de variação(cv), que também server para nos indicar a homogeneidade dos dados(quanto menor o c.v., maior a homogeneidade).Se o valor do cv for menor que 50%, então a média é representativa; não, caso contrário.
...
sprintf("o valor do do coeficiente de variação para as distâncias da operação add é: %f%%", cv_add)
...
sprintf("o valor do do coeficiente de variação para as distâncias da operação deletar é: %f%%", cv_del)
